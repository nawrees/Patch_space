...
/**
 * Professional inline-SVG icon set (Lucide-style stroke icons).
 * Renders monochrome icons that inherit the surrounding text color via
 * `currentColor`, so they adapt automatically to the light/dark theme.
 *
 * Usage:  <app-icon name="flame"></app-icon>
...
export class IconComponent {
  private _name = '';
  private _cache: SafeHtml | null = null;
  ...
  get svg(): SafeHtml {
    ...
    const inner = ICONS[this._name] || ICONS['circle'];
    const filled = FILLED.has(this._name);
    const attrs = filled
      ? 'fill="currentColor" stroke="none"'
      : 'fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"';
    const html = `<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" ${attrs}>${inner}</svg>`;
    this._cache = this.sanitizer.bypassSecurityTrustHtml(html);
    return this._cache;
  }
}

/** Icons drawn with fill instead of stroke. */
const FILLED = new Set<string>(['star-filled', 'stop', 'play']);

/** Inner SVG markup for each icon (24x24 viewBox). */
const ICONS: Record<string, string> = {
  flame: '<path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/>',
  'heart-crack': '<path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><polyline points="12 5 10 9 13 11 11 15"/>',
